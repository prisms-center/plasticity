  ...
  for (;cell!=endc; ++cell){
	...
	if (cell->face(f)->at_boundary()){
	  ...
	  if (face_center[0] == 0.0){
	    cell->face(f)->set_boundary_id (1); //back boundary
	  }
	  else if(face_center[0] == 1.0){
	    cell->face(f)->set_boundary_id (2); //front boundary
	  }
	  else if(face_center[1] == 0.0){
	    cell->face(f)->set_boundary_id (3); //left boundary
	  }
	  else if(face_center[2] == 0.0){
	    cell->face(f)->set_boundary_id (4); //bottom boundary
	  }
	}
      }
    }
