...
void crystalPlasticity<dim>::calculatePlasticity(unsigned int cellID,
  unsigned int quadPtID, unsigned int StiffnessCalFlag)
  {
    ...
    FullMatrix<double> Dmat2(2*dim,2*dim),TM(dim*dim,dim*dim), ElasticityTensor(2*dim,2*dim);
    ...
    Vector<double> s_beta(n_Tslip_systems),h_beta(n_Tslip_systems),delh_beta_dels(n_Tslip_systems),h0(n_Tslip_systems),a_pow(n_Tslip_systems),s_s(n_Tslip_systems);
    ...
    while (iter1) {
      ...
      for(unsigned int i=0;i<n_Tslip_systems;i++){
        ...
        for(unsigned int j=0;j<n_Tslip_systems;j++){
          ...
          for(unsigned int k=0;k<dim;k++){
            for(unsigned int l=0;l<dim;l++){
              if(((resolved_shear_tau(i)-W_kh_tau(i))*(resolved_shear_tau(j) - W_kh_tau(j)))<0.0)
      ...
      for (unsigned int i=0;i<n_twin_systems;i++){//
        ...
        if((s_alpha_tau(n_slip_systems+i)>(saturationStressTwin[i]))&&(twin_conv[cellID][quadPtID] != 1.0)){
      ...
      if (n_PA > 0) {
        ...
        for (unsigned int alpha = 0;alpha<n_PA;alpha++) {
          ...
          for (unsigned int beta = 0;beta<n_PA;beta++) {
            ...
            if (((resolved_shear_tau_trial(iter)-W_kh_t(iter))<0.0) ^ ((resolved_shear_tau_trial(iter3) - W_kh_t(iter3))<0.0))
